	...
	function resizeImage( image, needsPowerOfTwo, needsNewCanvas, maxSize ) {
		...
		if ( scale < 1 || needsPowerOfTwo === true ) {
			...
			if ( ( typeof HTMLImageElement !== 'undefined' && image instanceof HTMLImageElement ) ||
				...
				if (!__reported['not-power-of-two']) {
					console.warn( 'THREE.WebGLRenderer: Texture has been resized from (' + image.width + 'x' + image.height + ') to (' + width + 'x' + height + ').' );
					__reported['not-power-of-two'] = 1;
				}
				...
				if ( 'data' in image ) {
					...
					// NOTE (MS) Don't warn about non-power of two textures all the time
					// TODO (AXC): should we be reporting cannot make power of two versus too large?
					if (!__reported['cannot-make-power-of-two']) {
						console.warn( 'THREE.WebGLRenderer: Image in DataTexture is too big (' + image.width + 'x' + image.height + ').' );
						__reported['cannot-make-power-of-two'] = 1;
